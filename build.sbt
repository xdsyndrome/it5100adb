...
slick := {
    ...
    val url = "jdbc:postgresql://[::1]:5433/postgres?user=postgres&password=123;INIT=runscript from 'src/main/sql/test.sql'" // connection info
    ...
    runner.value.run("slick.codegen.SourceCodeGenerator",
        cp.files,
        Array(profile, jdbcDriver, url, outputDir.getPath, pkg),
